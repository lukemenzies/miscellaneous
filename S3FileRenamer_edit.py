...
def perform_rename(old_name, root_path, found_bad_characters, rename_do, n):
...
#   corrected_name = re.sub("|".join(found_bad_characters), "", old_name)
    ...
    if rename_do:
        ...
        if os.path.exists(corrected_name):
            n += 1
            re.sub(corrected_name.split(".")[0], corrected_name.split(".")[0] + str(n), corrected_name)
...
if __name__ == "__main__":
    ...
    for root, dirs, files in walk(path, topdown=False):
        for name in files:
            ...
            if found is True:
                new_name = perform_rename(name, root, characters_found, do_rename, num)
